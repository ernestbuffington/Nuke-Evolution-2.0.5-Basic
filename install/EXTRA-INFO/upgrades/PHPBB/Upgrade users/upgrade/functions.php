...
function generate_config() {
    global $install_lang, $next_step;

    if(@is_file('config.php')) {
        @unlink('config.php');
    }

    $filename = "upgrade/config_blank.php";
	...
	$_SESSION['configData'] = $contents;
        echo("<input type='hidden' name='download_file' value='1'>"
            ...
            ."<tr><td align=\"center\"><b>" . $install_lang['config_failed'] . "</b></td></tr>"
...
function validate_data($post) {
    ...
    $dbpass = ($_POST['dbpass']) ? $_POST['dbpass'] : $error .= "<tr><td align=\"center\"><font color=red>" . $install_lang['dbpass_error'] . "</font></td></tr>";
...
function server_check() {
    ...
    if (isset($_SESSION['dbtype'])) {
        if ($_SESSION['dbtype'] != "MySQL" && $_SESSION['dbtype'] != "mysql") {
...
function do_sql($install_file) {
    global $nuke_name, $next_step, $step, $install_lang, $prefix;
    ...
    while ((!feof($fp) || strlen($buffer)))
    {
        ...
        elseif (!$inside_quote && $current_char == ';')
        {
            ...
            {
                $message .= "<tr><td align=\"center\">$data_buffer<br /><font color=red>" . $install_lang['sql_error'] . mysql_errno().": ".mysql_error()."</td></tr>";
            }
    ...
    $message .= "<tr><td align=center><input type=hidden name=step value=$next_step><input type=submit name=confirm value=\"" . $install_lang['configure'] . "\"></td></tr>";
...
function site_form($display = 1) {
    ...
    if (!empty($display)) {
        $form .=" <tr><td align=\"center\" width=50%><b>" . $install_lang['sitename'] . "</b></td> <td align=\"center\" width=50%><input type=text length=60 value=\"http://".$_SERVER["SERVER_NAME"]."\" name=\"sitename\"></td></tr>";

        $form .=" <tr><td align=\"center\" width=50%><b>" . $install_lang['sitedescr'] . "</b></td> <td align=\"center\" width=50%><input type=text length=60 value=\"$new[site_desc]\" name=\"site_desc\"></td></tr>";

        $form .=" <tr><td align=\"center\" width=50%><b>" . $install_lang['cookie_name'] . "</b></td> <td align=\"center\" width=50%><input type=text length=60 value=\"$new[cookie_name]\" name=\"cookie_name\"></td></tr>";

        $form .=" <tr><td align=\"center\" width=50%><b>" . $install_lang['cookie_path'] . "</b></td> <td align=\"center\" width=50%><input type=text length=60 value=\"$new[cookie_path]\" name=\"cookie_path\"></td></tr>";
        $form .=" <tr><td align=\"center\" width=50%><b>" . $install_lang['cookie_domain'] . "</b></td> <td align=\"center\" width=50%><input type=text length=60 value=\"".$_SERVER["HTTP_HOST"]."\" name=\"cookie_domain\"></td></tr>";
        $namechange_checked = ($new['allow_namechange']) ? "checked" : "";
        $namechange_not_checked = (!$new['allow_namechange']) ? "checked" : "";
        $form .=" <tr><td align=\"center\" width=50%><b>" . $install_lang['namechange'] . "</b></td> <td align=\"center\" width=50%>" . $install_lang['yes'] . " <input type=radio value=\"1\" name=\"allow_namechange\" $namechange_checked> " . $install_lang['no'] . " <input type=radio value=\"0\" name=allow_namechange $namechange_not_checked></td></tr>";
        $form .=" <tr><td align=\"center\" width=50%><b>" . $install_lang['email_sig'] . "</b></td> <td align=\"center\" width=50%><textarea name=board_email_sig>$new[board_email_sig]</textarea></td></tr>";

        $form .=" <tr><td align=\"center\" width=50%><b>" . $install_lang['site_email'] . "</b></td> <td align=\"center\" width=50%><input type=text name=\"board_email\" value=\"".$new[board_email]."\"></td></tr>";
        $form .=" <tr><td align=\"center\" width=50%><b>" . $install_lang['default_lang'] . "</b></td> <td align=\"center\" width=50%>" . language_select('english', 'default_lang') . "</td></tr>";
        $form .=" <tr><td align=\"center\" width=50%><b>" . $install_lang['server_name'] . "</b></td> <td align=\"center\" width=50%><input type=text name=\"server_name\" value=\"".$_SERVER["SERVER_NAME"]."\"></td></tr>";
        $form .=" <tr><td align=\"center\" width=50%><b>" . $install_lang['server_port']. "</b></td> <td align=\"center\" width=50%><input type=text name=\"server_port\" value=\"".$new[server_port]."\"></td></tr>";
