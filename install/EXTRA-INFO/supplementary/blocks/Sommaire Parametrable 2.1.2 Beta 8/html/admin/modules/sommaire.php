...
/*              Module Sommaire paramétrable for PHP-Nuke.               */
/*                            - 26 Mai  2004 -                           */
/*                                                                       */
/* Nom du Module: Sommaire paramétrable                                  */
/* Version du Module: 2.1.1                                              */
/*                                                                       */
/* Description du Module:                                                */
/*       Bloc Sommaire paramétrable : affichage par catégories, avec de  */
/*       nombreuses possibilités de présentation :                       */
/*       - Menu dynamique (catégories affichées/cachées avec un clic     */
/*       - Ajout manuel d'une icone 'NEW' devant les catégories          */
/*       - Ajout manuel d'une icone 'NEW' devant les liens/modules       */
/*       - Ajout automatique d'une icone 'NEW' devant les modules de     */
/*         base, avec la détection des nouveautés du site, et choix de   */
/*         la durée des "nouveautés".                                    */
/*       - Possibilité de mettre un lien ou une catégorie en gras        */
/*       - Contenu des catégories peut être affiché en listbox           */
/*       - Choix du style des liens/modules (classe css)                 */
/*       - Affichage possible des catégories avec uniquement une image   */
/*       - Affichage possible des liens/modules avec uniquement une image*/
/*       - Barre horizontale pour séparer 2 catégories ou 2 liens (v2.1) */
/*       - Titre de catégorie centré                                     */
/*       - Couleur de fond différente pour chaque catégorie              */
/*       - Image sur les catégories et les modules                       */
/*       - Ajout de liens externe (n'importe quelle url)                 */
/*       - Affichage d'une icône dans le sommaire si le membre a des     */
/*         Messages Privés non lus.                                      */
/*       - Masquer ou non aux visiteurs les modules réservés aux membres */
/*       - Choisir le style (dans style.css) des noms de catégories      */
/*       - Mettre un fichier FLASH .swf à la place du nom de catégorie   */
/*         (FLASH implémenté par MAC06 - magetmac06@hotmail.com)         */
/*                                                                       */
/* License: GNU/GPL                                                      */
...
<?php // Fonction permettant d'afficher le popup pour les options supplémentaires?>
    function envoiedit(keysommaire, z) {
        ...
        var zeurl="<?php echo $admin_file;?>"+".php?op=sommaire&amp;go=edit&amp;modulename="+modulename+"&amp;lienname="+lienname+"&amp;image="+image+"&amp;catname="+catname+"&amp;catimage="+catimage+"&amp;categoryclass="+categoryclass+"&amp;lienclass="+lienclass+"&amp;new_days="+new_days+"&amp;keysommaire="+keysommaire+"&amp;z="+z;
...
// si l'image de catégorie indiquée dans la DB est "noimg" (--> pas d'image!), on affiche l'image 'null.gif', qui est une image transparente.
...
// fonction jscript permettant de changer l'image affichée (en fonction de la valeur indiquée dans la listbox 'image dans cette catégorie')
...
// combien y a-t-il de modules dans cette catégorie ?
        ...
        for ($z=0;$z<$nombremodules;$z++) {
            ...
            echo "<tr id=\"span$formpointeur\"><td valign=\"center\"><img src=\"".$urlofimages."/$afficheimageinthiscategorie\" name=\"image$formpointeur\">"
                // on va changer l'image affichée devant le nom du module...
                ."<script language=\"Javascript\">"
...
if ($z==$nombremodules-1) { // si on est à la dernière listbox, on affiche le message demandant d'envoyer les modifs.
    ...
    // si on sélectionne 'Lien externe' dans la liste des modules, cela va afficher les inputbox.
            ...
            if ($moduleinthisgroup[$groupmenu[$keysommaire]][$z]=="Lien externe") { // si 'Lien externe' est indiqué dans la DB, on affiche les inputbox, sinon on les masque par défaut
                ...
                if ($testehttp===0 || $testeftp===0 || $testehttps===0) { //permet d'afficher l'image "ouvrir dans une nouvelle fenêtre" si le lien commence par http:// ou ftp://
...
//echo "{$imageinthisgroup[$groupmenu[$keysommaire]][$z]}<br />";
    ...
    //        echo "[<a href='javascript:envoiedit(document.forms.form_sommaire.elements[\"sommaireformingroup[".$keysommaire."][".$z."]\"].value, document.forms.form_sommaire.elements[\"sommaireformmodulelink[".$keysommaire."][".$z."]\"].value, document.forms.form_sommaire.elements[\"sommaireformmodulelinktext[".$keysommaire."][".$z."]\"].value, document.forms.form_sommaire.elements[\"sommaireformmoduleimage[".$keysommaire."][".$z."]\"].value, document.forms.form_sommaire.elements[\"sommaireformmodulegras[".$keysommaire."][".$z."]\"].value, document.forms.form_sommaire.elements[\"sommaireformmodulenew[".$keysommaire."][".$z."]\"].value, ".$idofthismodule[$groupmenu[$keysommaire]][$z].",\"".$catname[$keysommaire]."\", document.forms.form_sommaire.elements[\"sommaireformimage[".$keysommaire."]\"].value, \"".$groupmenu[$keysommaire]."\", ".$keysommaire.", ".$z.")' title=\""._SOMMOREOPTIONS."\">+</a>]";
        ...
        if ($z>$nbmodules) { // pour n'afficher qu'une seul liste déroulante vide, on cache les autres.
            echo "<script language=\"JavaScript\">"
...
// à chaque ligne du formulaire, on fait une requête pour insérer les données.
    for ($i=0; $i<=$sommaireformkeysommaire; $i++) {
        for ($j=0; $j<count($sommaireformingroup[$i]); $j++) {
            ...
            $zeclass = (empty($zeclass)) ? $sommaireformclassformodules : $zeclass ; //si la classe est vide, alors on remplit avec la classe par défaut.
            ...
            $zenew_days = (empty($zenew_days)) ? $sommaireformnew_days : $zenew_days ; //si le nb de jour est vide, alors on remplit avec le nb de jours par défaut.
            if ($sommaireformingroup[$i][$j] =="Lien externe") { //si il y a un lien, on insère les données du lien
/*****[BEGIN]******************************************
 [ Base:    Caching System                     v3.0.0 ]
 ******************************************************/
                $cache->delete('sommaire_row', 'block');
...
// si la catégorie ne contient aucune donnée (complètement vide), alors on ne l'insère pas dans la DB.
        if ($sommaireformname[$i]=='' && $sommaireformimage[$i]=="noimg" && $sommaireformlien[$i]=='' && $sommaireformhr[$i]=='' && $sommaireformcenter[$i]=='' && $sommaireformbgcolor[$i]=='') {
            ...
            $zeclass = (empty($zeclass)) ? $sommaireformclass : $zeclass ; //si la classe est vide, alors on remplit avec la classe par défaut.
