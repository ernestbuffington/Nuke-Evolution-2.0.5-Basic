...
/************************************************************************************/
            ...
            if (eregi(".swf",$som_image) || empty($som_name) || $som_name==" " ||$som_name=="&nbsp;" ||$som_name=="&amp;nbsp;") { //////////////////// support des fichiers FLASH - par MAC06 -+- marcoledingue : ajout du second check, qui permet d'avoir des catégories avec un nom vide. //////////////////////////
        ...
        while ($moduleinthisgroup[$som_groupmenu][$keyinthisgroup]) { //on va checker si chaque module indiqué dans la catégorie en cours est installé et activé/visible //
                ...
                elseif($moduleinthisgroup[$som_groupmenu][$keyinthisgroup]!="SOMMAIRE_HR") {
                    for ($z=0;$z<count($module);$z++) { //pour chaque module activé et visible on va regarder où on l'affiche
                        if ($module[$z]!=$main_module && (($is_admin===1 AND $view[$z] == 2) OR $view[$z] != 2) && $moduleinthisgroup[$som_groupmenu][$keyinthisgroup]==$module[$z]) {
                            ...
                            $customtitle2 = (!empty($customtitle[$z])) ? $customtitle[$z] : ereg_replace("_", " ", $module[$z]);
            ...
            elseif($moduleinthisgroup[$som_groupmenu][$keyinthisgroup]=="Lien externe" && !eregi("^modules.php\?name=", $linkinthisgroup[$som_groupmenu][$keyinthisgroup]) && !eregi("^((http(s)?)|(ftp(s)?))://".$_SERVER['SERVER_NAME']."/modules.php\?name=",$linkinthisgroup[$som_groupmenu][$keyinthisgroup])) { // gestion des liens externes - v2.1.2beta5 : ajout d'un check supplémentaire pour gérer les liens externes (target blank mais sur le serveur)
                     // gestion multilingue : si le lien commence par 'LANG:_' alors c'est multilingue, donc on va afficher ce qui a été inscrit dans le fichier de langue.
                if (strpos($linkinthisgroup[$som_groupmenu][$keyinthisgroup],"LANG:_")===0) {
                    $zelink_lang = str_replace("LANG:","",$linkinthisgroup[$som_groupmenu][$keyinthisgroup]);
                    eval( "\$zelink_lang = $zelink_lang;");
            ...
            elseif ($moduleinthisgroup[$som_groupmenu][$keyinthisgroup]=="SOMMAIRE_HR") {
                ...
                for ($z=0;$z<count($module);$z++) { //pour chaque module activé et visible on va regarder où on l'affiche
                    if ($moduleinthisgroup[$som_groupmenu][$keyinthisgroup]=="Lien externe") { //si c'est un lien externe, il commence par 'modules.php?name=' ==>c'est un lien vers un module du site
                        ...
                        if (ereg("^((http(s)?)|(ftp(s)?))://".$_SERVER['SERVER_NAME']."/modules.php\?name=",$linkinthisgroup[$som_groupmenu][$keyinthisgroup])) { // v2.1.2beta5 : les liens externes target blank qui pointent vers le serveur sont traités comme des modules.
                            $nomdumodule = substr(strstr($temponomdumodule[0],'modules.php'),17);
                            $targetblank="target=\"_blank\"";
                        }
                        elseif (eregi("^((http(s)?)|(ftp(s)?))://".$_SERVER['SERVER_NAME']."/modules.php\?name=",$linkinthisgroup[$som_groupmenu][$keyinthisgroup])) { // v2.1.2beta6 : les liens externes target blank qui pointent vers le serveur sont traités comme des modules.
                            $nomdumodule = substr(strstr($temponomdumodule[0],'modules.php'),17);
                            $targetblank="";
                        }
                        else {
                        ...
                        if (($is_admin===1 AND $view[$z] == 2) OR $view[$z] != 2) { //si on n'est pas admin et que le module est réservé aux admins, il n'apparaît pas
                            if ($nomdumodule==$module[$z]) {
                                if ($dynamic==1 && $detectMozilla!=1) {
                                    ...
                                    if (ereg(addcslashes("$urldumodule$", '?&'), $requesturi)) { // si la page visualisée est le module[$z], alors on récupère son groupmenu pour ne pas enrouler la catégorie par défaut.
                                ...
                                elseif ($is_user==0 AND $view[$z]==1 AND $invisible[0]==3 && $is_admin==0) { //c'est un visiteur, il doit �tre membre pour voir le module (pas de gestion des groupes)
                                    ...
                                    elseif ($nomdumodule=="Reviews" && $newdaysinthisgroup[$som_groupmenu][$keyinthisgroup]!="-1") {
                                        ...
                                        $rowimgnew = $db->sql_fetchrow($resultimgnew);
                                        if ($rowimgnew['date']) {
                                            ereg ("([0-9]{4})-([0-9]{1,2})-([0-9]{1,2})", $rowimgnew['date'], $datetime);
