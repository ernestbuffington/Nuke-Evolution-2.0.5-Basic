...
include('../../../includes/functions_selects.'.$phpEx);
 ...
 ******************************************************/
        ...
        else if ( intval($id) != 0 && $s == 'pd' || $s == 'td' )
        {
            ...
            if ( $postrow = $db->sql_fetchrow($approve_result) )
            {
                ...
                $poster_joined = ( $postrow['user_id'] != ANONYMOUS ) ? $lang['Joined'] . ': ' . $postrow['user_regdate'] : '';
                $poster_avatar = '';
                ...
                $temp_url = append_sid("modcp.$phpEx?mode=ip&amp;" . POST_POST_URL . "=" . $postrow['post_id'] . "&amp;" . POST_TOPIC_URL . "=" . $topic_id . "&amp;sid=" . $userdata['session_id']);
...
if ( $modevar['u'] == true )
{
        ...
        if ( $approve_mod['um'] == true )
        {
            ...
            for($i = 1; $i <= intval($approve_page_count); $i++)
            {
                ...
                {
                    $template->assign_block_vars("moderated.moderated_paginate", array(
                            "S_LINK" => $prev . "<strong>" . $i . "</strong>" . $next
                        )
                    );
                }
                else
...
/*****[BEGIN]******************************************
 [ Mod:    Advanced Username Color             v1.0.5 ]
 ******************************************************/
                            "S_USERNAME" => UsernameColor($approve_row2['username']),
/*****[END]********************************************
 [ Mod:    Advanced Username Color             v1.0.5 ]
 ******************************************************/
...
/*****[BEGIN]******************************************
 [ Mod:    Advanced Username Color             v1.0.5 ]
 ******************************************************/
                            "S_AUTHOR" => UsernameColor($approve_row2['username']),
/*****[END]********************************************
 [ Mod:    Advanced Username Color             v1.0.5 ]
 ******************************************************/
