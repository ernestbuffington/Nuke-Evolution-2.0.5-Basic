...
$sqlo = "SELECT * FROM `".$prefix."_MA_mapp` WHERE formno = $formno AND parent = 0 ORDER BY fldord ASC";
if ( !($resulto = $db->sql_query($sqlo)) )
{
  echo "ERROR - 19A1 - ".MA_UTOQTERROR."! <br>";
  exit();
}

$num_rowso = $db->sql_numrows($resulto);
if ((($onum<2) && ($mydir < 0)) || (($onum >= ($num_rowso)) && ($mydir > 0)))
{
  Header("Location: ".$admin_file.".php?op=MAlistpq&formno=$formno");
  break;
}    
...
if (!$resulto)
{
  echo "ERROR - 19A2 - ".MA_UTUQTERROR."! <br>";
  exit();
}
...
if (!$resultou)
{
  echo "ERROR - 19A3 - ".MA_UTUQTERROR."! <br>";
  exit();
}
