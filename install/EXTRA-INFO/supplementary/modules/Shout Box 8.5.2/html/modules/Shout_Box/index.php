...
global $db, $user, $cookie, $prefix, $username, $nsnst_const;
cookiedecode();
$username = $cookie[1];
...
function searchHistory($where, $sbsearchtext, $results, $style, $timeframe, $order) {
    ...
    if ((empty($order)) OR ($order == 'newest')) { $sql .= " ORDER BY id DESC"; }
...
//     if (($timeframe == 0) OR (empty($timeframe))) { $tfSEL0 = " selected=\"selected\""; } else { $tfSEL0 = ""; }
