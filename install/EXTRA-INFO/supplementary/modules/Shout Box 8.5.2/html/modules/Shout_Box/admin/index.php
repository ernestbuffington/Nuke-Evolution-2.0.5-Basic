...
function manageShouts($page, $pruned) {
    global $prefix, $db, $admin, $admin_file, $sbURL, $user, $cookie, $module_name, $user_prefix;
...
function pruneSubmit($pruneDays, $page) {
    ...
    if (empty($pruned) OR $pruned < 0) { $pruned = 0; }
...
function ShoutBoxLayout() {
    ...
    } elseif ((!empty($v)) AND ($v <= $row['version'])) {
...
function shoutHealth($SBhealthCount) {
    ...
    $rowCheck = $db->sql_fetchrow($resultCheck);
    if ($rowCheck['active'] != 1) {
        echo "<p class=\"content\"><strong>Shout Box module not active:</strong> $module_name.php&nbsp;&nbsp;<strong>Recommendation:</strong> Click the Modules icon in PHP-Nuke administration. In the Functions column and Shout Box row, choose 'Activate'.</p>";
    }
    if (($rowCheck['view'] != 0) AND ($rowCheck['view'] != 1)) {
...
function ShoutBoxThemeing() {
    global $prefix, $db, $admin_file, $sbURL, $module_name;
    include_once(NUKE_BASE_DIR.'header.php');
    LinkAdmin();
    $ThemeSel = get_theme();
    $sql = "SELECT * FROM ".$prefix."_shoutbox_themes WHERE themeName='$ThemeSel'";
    ...
    while ($image=$imagesDir->read()) {
        if (!empty($image) AND $image != '.' AND $image != '..' AND $image != 'index.html' AND file_exists(NUKE_MODULES_DIR.$module_name."/images/down/$image") AND file_exists(NUKE_MODULES_DIR.$module_name."/images/pause/$image")) {
            $imageList .= "$image ";
        }
    }
    ...
    while ($backImage=$backgroundDir->read()) {
        if (!empty($backImage) AND $backImage != '.' AND $backImage != '..' AND $backImage != 'index.html') {
