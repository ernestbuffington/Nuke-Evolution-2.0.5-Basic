...
include_once("../../../includes/functions_jr_admin.$phpEx");
...
/** Functions
/***************************************************************************/
$page_link = ($_GET['page']) ? "&page=" . $_GET['page'] : "";
...
{
        $sql = "SELECT user_color_gi, user_color_gc, username, user_id, user_level  FROM " . USERS_TABLE . "
        ...
        if (!empty($color_group))
        {
                ...
                {
                        $selected = ($row['user_color_gi'] == $crow['group_id']) ? 'SELECTED' : '';
                        ...
                        if ($row['user_color_gi'] == $crow['group_id'])
                        {
...
else
{
        ...
        if (isset($HTTP_POST_VARS['update_user']) && !empty($user_id))
        {
                ...
                {
                        $sql = "SELECT group_color FROM " . $prefix . "_bbadvanced_username_color
                        WHERE group_id = $color_group_id";
                        if (!$result = $db->sql_query($sql))
                        {
                                message_die(GENERAL_ERROR, "Could not gather color info", '', __LINE__, __FILE__, $sql);
                        }
                        $color = $db->sql_fetchrow($color_result);
                        $color = $color['group_color'];

                        $sql = "UPDATE " . USERS_TABLE . "
                        SET user_color_gi = '$color_group_id', user_color_gc = '$color'
                        WHERE user_id = '$user_id'";
                        if (!$db->sql_query($sql))
                        {
                                message_die(GENERAL_ERROR, $lang['Error_User_Table'], '', __LINE__, __FILE__, $sql);
        ...
        $count = "SELECT user_id FROM " . USERS_TABLE . " WHERE user_id <> " . ANONYMOUS;
        if (!$count_result = $db->sql_query($count)) {
                message_die(GENERAL_ERROR, $lang['Error_User_Table'], '', __LINE__, __FILE__, $sql);
        }
        $count = $db->sql_numrows($count_result);

        $limit = 50;

        if((!$page) || (is_numeric($page) == false) || ($page < 0) || ($page > $count)) {
                  $page = 1;
        }
        ...
        $sql = "SELECT user_color_gi, user_color_gc, username, user_id, user_rank, user_allow_pm, user_allowavatar, user_active
                FROM " . USERS_TABLE . "
                WHERE user_id <> " . ANONYMOUS . "
                ORDER BY $sort_item $order LIMIT $set_limit, $limit";
        if (!$result = $db->sql_query($sql))
        {
                message_die(GENERAL_ERROR, $lang['Error_User_Table'], '', __LINE__, __FILE__, $sql);
        }
                ...
                if($prev_page >= 1) {
                  $link = append_sid(basename(__FILE__) . "?page=$prev_page");
                  $pages = "<a href=\"$link\"><b><< Prev</b></a>";
                }

                for($a = 1; $a <= $total_pages; $a++)
                {
                $thing = (($a + 1) > $total_pages) ? "" : " | ";

                   if($a == $page) {
                          $pages .= "<b> $a</b>$thing";
                         } else {
                  $link = append_sid(basename(__FILE__) . "?page=$a");
                  $pages .= "  <a href=\"$link\"> $a </a>$thing";
                         }
                }

                $next_page = $page + 1;
                if($next_page <= $total_pages) {
                  $link = append_sid(basename(__FILE__) . "?page=$next_page");
                   $pages .= "<a href=\"$link\"><b>Next >></b></a>";
                }
        ...
        while ($row = $db->sql_fetchrow($result))
        {
                ...
                if (!empty($color_group))
                {
                        if ($row['user_color_gi'] != 0)
                        {
                                ...
                                $sql = 'SELECT * FROM ' . $prefix . '_bbadvanced_username_color
                                WHERE group_id = ' . $row['user_color_gi'];
                                if (!$cresult = $db->sql_query($sql))
                                {
                                        message_die(GENERAL_ERROR, $lang['Error_User_Table'], '', __LINE__, __FILE__, $sql);
                                }
        ...
        'IMG_COLORGROUP' => ($sort_item == 'user_color_gi') ? ($order == 'ASC') ? $asc_img : $desc_img : '',
        ...
        'S_COLORGROUP' => $base_filename . '&sort_item=user_color_gi',
...
'S_PROFILE' => append_sid('../../../profile.'.$phpEx),
