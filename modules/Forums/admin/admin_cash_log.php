...
if ( !empty($setmodules) )
{
	...
	$phpbb['Cash Mod']['Cash_Logs'] = $file . "?saction=all&stime=all&scount=b&sindex=0";
	return;
}
...
$pagination = generate_pagination("admin_cash_log.$phpEx?saction=$saction&amp;stime=$stime&amp;scount=$scount", max(1,$total), $ar_count[$scount], $start);
...
while ( list($key,) = each ( $ar_action ) )
{
	$template->assign_block_vars("actionfilter", array(	"ROW_CLASS" => (( !($i % 2) ) ? $theme['td_class1'] : $theme['td_class2']),
														...
														"NAME" => $lang['Cash_' . $key],
														"LINK" => append_sid("admin_cash_log.$phpEx?saction=$key&stime=$stime&scount=$scount&sindex=0"),
														"DELETE" => $lang['Delete_' . $key . '_logs'],
														"DELETECOMMAND" => append_sid("admin_cash_log.$phpEx?delete=$key")));
	if ( $key != $saction )
	{
...
while ( list($key,) = each ( $ar_time ) )
{
	$template->assign_block_vars("timefilter", array(	"ROW_CLASS" => (( !($i % 2) ) ? $theme['td_class1'] : $theme['td_class2']),
														...
														"LINK" => append_sid("admin_cash_log.$phpEx?saction=$saction&stime=$key&scount=$scount&sindex=0")));
	if ( $key != $stime )
	{
...
{
	$template->assign_block_vars("countfilter",array("NAME" => $number, "LINK" => append_sid("admin_cash_log.$phpEx?saction=$saction&stime=$stime&scount=$key&sindex=0")));
	if ( $key != $scount )
	{
