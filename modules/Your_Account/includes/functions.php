...
function ya_userCheck($username) {
    ...
    //if ((!$username) || (empty($username)) || (ereg("['^À-ÿa-zA-Z0-9_-']",$username))) $stop = "<center>"._ERRORINVNICK."</center><br />";
    //if ((!$username) || (empty($username)) || (ereg("['^a-zA-Z0-9['.['.'] [.'].']_{}«»=|-']",$username))) $stop = "<center>"._ERRORINVNICK."</center><br />";
...
function ya_mailCheck($user_email) {
    ...
    if ($ya_config['bad_mail'] > "") {
        ...
        for ($i=0; $i < count($BadMailList); $i++) {
            if (eregi($BadMailList[$i], $user_email)) $stop = "<center>"._MAILBLOCKED." <strong>".$BadMailList[$i]."</strong></center><br />";
        }
    }
