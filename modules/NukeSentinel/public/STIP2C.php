...
if ($totalselected > 0) {
  ...
  echo "<td align='right' bgcolor='$bgcolor2' colspan='5'><strong>"._AB_SORT.":</strong> ";
  ...
  if($column == "date") $selcolumn4 = "selected";
  ...
  while ($getIPs = $db->sql_fetchrow($result)) {
    ...
    list($getIPs['country']) = $db->sql_fetchrow($db->sql_query("SELECT * FROM `".$prefix."_nsnst_countries` WHERE `c2c`='".$getIPs['c2c']."'"));
  ...
  if ($pages != 1 && $pages != 0) {
    ...
    echo "<input type='hidden' name='op' value='STIP2C'>\n";
    echo "<td align='right' bgcolor='$bgcolor2' colspan='5'><strong>"._AB_PAGE."</strong> ";
    $counter = 1;
