...
function singlecomment($tid, $sid, $mode, $order, $thold) {
    ...
    echo "<table width=\"99%\" border=\"0\"><tr bgcolor=\"$bgcolor1\"><td width=\"500\">";
    formatTimestamp($date);
    if($email) echo "<strong>$subject</strong> <font class=\"content\" color=\"$textcolor2\">("._SCORE." $score)<br />"._BY." <a href=\"mailto:$email\"><font color=\"$bgcolor2\">$name</font></a> <font class=content><strong>($email)</strong></font> "._ON." $datetime";
    else echo "<strong>$subject</strong> <font class=content>("._SCORE." $score)<br />"._BY." $name "._ON." $datetime";
