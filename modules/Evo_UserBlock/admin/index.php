...
function evouserinfo_drawlists () {
    global $lang_evo_userblock, $admin_file, $Default_Theme, $module_name, $board_config, $userinfo, $evouserinfo_ec;
    ...
    //Breaks
    $end = count($inactive) + count($active);
    for ($i = 0; $i < $end; $i++) {
        ...
        foreach ($data as $type => $sub) {
            if ($type == 'left_col') {
                ...
                foreach ($sub as $element) {
                    if (!ereg($lang_evo_userblock['ADMIN']['BREAK'],$element)) {
