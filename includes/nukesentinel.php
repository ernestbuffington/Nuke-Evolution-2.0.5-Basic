...
if (file_exists(NUKE_BASE_DIR.'language/nukesentinel/lang-'.$currentlang.'.php')) {
   include_once(NUKE_BASE_DIR.'language/nukesentinel/lang-'.$currentlang.'.php');
} else {
   include_once(NUKE_BASE_DIR.'language/nukesentinel/lang-english.php');
}
...
if($ab_config['track_active'] == 1 AND !is_excluded($nsnst_const['remote_ip'])) {
  ...
  if($pg != "/rss.php" AND $pg != '/modules.php' AND !stristr($pg, "op=gfx")) {
    ...
    $result = $db->sql_query("INSERT INTO `".$prefix."_nsnst_tracked_ips` (`user_id`, `username`, `date`, `ip_addr`, `ip_long`, `page`, `user_agent`, `x_forward_for`, `client_ip`, `remote_addr`, `remote_port`, `request_method`, `c2c`) VALUES ('".$nsnst_const['ban_user_id']."', '$ban_username2', '".$nsnst_const['ban_time']."', '".$nsnst_const['remote_ip']."', '".$nsnst_const['remote_long']."', '$pg', '$user_agent', '".$nsnst_const['forward_ip']."', '".$nsnst_const['client_ip']."', '".$nsnst_const['remote_addr']."', '".$nsnst_const['remote_port']."', '".$nsnst_const['request_method']."', '$c2c')");
	$db->sql_freeresult($result);
