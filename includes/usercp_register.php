 ...
 *****************************************************/
    $strip_var_list = array('email' => 'email', 'myspace' => 'myspace', 'icq' => 'icq', 'bday_day' => 'bday_day', 'bday_month' => 'bday_month', 'bday_year' => 'bday_year', 'aim' => 'aim', 'msn' => 'msn', 'yim' => 'yim', 'website' => 'website', 'location' => 'location', 'occupation' => 'occupation', 'interests' => 'interests', 'confirm_code' => 'confirm_code', 'glance_show' => 'glance_show');
/*****[END]********************************************
 [ Mod:    At a Glance Options                 v1.0.0 ]
 *****************************************************/
 ...
 ******************************************************/ 
    ...
    validate_optional_fields($icq, $myspace, $aim, $msn, $yim, $website, $location, $occupation, $interests, $signature);
...
/*****[BEGIN]******************************************
 ...
 [ Mod:     Hide Images                        v1.0.0 ]
 ******************************************************/
            $sql = "INSERT INTO " . USERS_TABLE . " (user_id, username, user_regdate, user_password, user_email, user_myspace, user_birthday, user_icq, user_website, user_occ, user_from, user_from_flag, user_interests, user_glance_show, user_sig, user_sig_bbcode_uid, user_avatar, user_avatar_type, user_viewemail, user_aim, user_yim, user_msnm, user_attachsig, user_allowsmile, user_showavatars, user_showsignatures, user_allowhtml, user_allowbbcode, user_allow_viewonline, user_notify, user_notify_pm, user_allow_mass_pm, user_popup_pm, user_timezone, user_time_mode, user_dst_time_lag, user_dateformat, user_show_quickreply, user_quickreply_mode, user_wordwrap, user_open_quickreply, user_lang, theme, user_level, user_allow_pm, name, newsletter, bio, user_hide_images, user_active, user_actkey, user_gender)
            VALUES ('$user_id', '" . str_replace("\'", "''", $username) . "', '" . $reg_date . "', '" . str_replace("\'", "''", $new_password) . "', '" . str_replace("\'", "''", $email) . "', '" . str_replace("\'", "''", $myspace) . "', " . sprintf('%02d%02d%04d',$bday_month,$bday_day,$bday_year) . ", '" . str_replace("\'", "''", $icq) . "', '" . str_replace("\'", "''", $website) . "', '" . str_replace("\'", "''", $occupation) . "', '" . str_replace("\'", "''", $location) . "', '$user_flag', '" . str_replace("\'", "''", $interests) . "', '" . str_replace("\'", "''", $glance_show) . "', '" . str_replace("\'", "''", $signature) . "', '$signature_bbcode_uid', '$user_avatar', '$user_avatar_type', '$viewemail', '" . str_replace("\'", "''", str_replace(' ', '+', $aim)) . "', '" . str_replace("\'", "''", $yim) . "', '" . str_replace("\'", "''", $msn) . "', '$attachsig', '$allowsmilies', '$showavatars', '$showsignatures', '$allowhtml', '$allowbbcode', '$allowviewonline', '$notifyreply', '$notifypm', '$allow_mass_pm', '$popup_pm', '$user_timezone', '$time_mode', '$dst_time_lag', '" . str_replace("\'", "''", $user_dateformat) . "', '$user_show_quickreply', '$user_quickreply_mode', '" . str_replace("\'", "''", $user_wordwrap) . "', $user_open_quickreply, '" . str_replace("\'", "''", $user_lang) . "', '$user_style', '$gender', '1', '1', '".$realname."', '".$newsletter."', '".$extra_info."', '$hide_images', ";
/*****[END]********************************************
 [ Mod:     Hide Images                        v1.0.0 ]
 [ Mod:     At a Glance Options                v1.0.0 ]
 [ Mod:     Signature Editor/Preview Deluxe    v1.0.0 ]
...
/*****[BEGIN]******************************************
 ...
 [ Mod:     Hide Images                        v1.0.0 ]
 ******************************************************/
    display_avatar_gallery($mode, $avatar_category, $user_id, $email, $current_email, $coppa, $username, $email, $new_password, $cur_password, $password_confirm, $myspace, $icq, $aim, $msn, $yim, $website, $location, $user_flag, $occupation, $interests, $glance_show, $signature, $viewemail, $notifypm, $allow_mass_pm, $popup_pm, $notifyreply, $attachsig, $allowhtml, $allowbbcode, $allowsmilies, $showavatars, $showsignatures, $allowviewonline, $user_style, $user_wordwrap, $user_lang, $bday_month, $bday_day, $bday_year, $user_timezone, $time_mode, $dst_time_lag, $user_dateformat, $user_show_quickreply, $user_quickreply_mode, $user_open_quickreply, $userdata['session_id'], $xdata, $rname, $extra_info, $newsletter, $hide_images, $gender);
/*****[END]********************************************
 [ Mod:     Hide Images                        v1.0.0 ]
 [ Mod:     At a Glance Options                v1.0.0 ]
 [ Mod:     XData                              v1.0.3 ]
...
// FLAGHACK-start
	...
	$flag_select = "<select name=\"user_flag\" onChange=\"document.images['user_flag'].src = 'modules/Forums/images/flags/'
 + this.value;\" >";
