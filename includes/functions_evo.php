...
function security_code($gfxchk, $size='normal', $force=0) {
    ...
    if (defined('CAPTCHA')) {
        ...
        switch($size) {
            case 'large':
            $code .= "<tr><td>"._SECURITYCODE.":</td><td><img src='index.php?op=gfx' border='0' alt='"._SECURITYCODE."' title='"._SECURITYCODE."'></td></tr>\n";
            $code .= "<tr><td>"._TYPESECCODE.":</td><td><input type=\"text\" name=\"gfx_check\" size=\"10\" maxlength=\"10\" AutoComplete=\"off\"></td></tr>\n";
            break;
            case 'normal':
            $code .= "<tr><td>"._SECURITYCODE.":</td></tr><tr><td><img src='index.php?op=gfx' border='0' alt='"._SECURITYCODE."' title='"._SECURITYCODE."'></td></tr>\n";
            $code .= "<tr><td>"._TYPESECCODE.":</td></tr><tr><td><input type=\"text\" name=\"gfx_check\" size=\"10\" maxlength=\"10\" AutoComplete=\"off\"></td></tr>\n";
            break;
            case 'small':
            $code .= _SECURITYCODE.": <img src='index.php?op=gfx' border='0' alt='"._SECURITYCODE."' title='"._SECURITYCODE."'>\n";
            $code .= _TYPESECCODE.": <input type=\"text\" name=\"gfx_check\" size=\"10\" maxlength=\"10\" AutoComplete=\"off\">\n";
            break;
            case 'stacked':
            $code .= _SECURITYCODE."<br /><img src='index.php?op=gfx' border='0' alt='"._SECURITYCODE."' title='"._SECURITYCODE."'><br />\n";
            $code .= _TYPESECCODE." <br /><input type=\"text\" name=\"gfx_check\" size=\"10\" maxlength=\"10\" AutoComplete=\"off\">\n";
            break;
            case 'demo':
...
function NetworkBar() {
    $content .= "<script>\r\n";
    $content .= "function change(url) {\r\n";
    $content .= "    if(url != \"\") {\r\n";
    $content .= " location = url;\r\n";
    $content .= "    }\r\n";
    $content .= "}\r\n";
    $content .= "</script>\r\n";
    $content .= "<table width=\"100%\" height=\"20\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\">\r\n";
    $content .= "    <tr valign=\"middle\">\r\n";
    $content .= "        <td>\r\n";
    $content .= "            <a href=\"http://nuke-evolution.com\" target=\"_blank\"><img src=\"images/evo-network.gif\" width=\"245\" height=\"20\" border=\"0\" alt=\"\"></a></td>\r\n";
    $content .= "        <td width=\"100%\" height=\"20\" align=\"right\" valign=\"middle\" background=\"images/evo-networkbg.gif\">\r\n";
    
    $news = array();
    $links = array();
    $networkData = eval(@file_get_contents('http://nuke-evolution.com/networkData.txt'));
    $num = array_rand($news);
    $nNews = $news[$num];
    $nNews = explode('|', $nNews);
    $title = $nNews[0];
    $url = $nNews[1];
    $content .= "            <a href=\"" . $url . "\"><span style=\"color: #D4D4D4;\">" . $title . "</span></a> \r\n";
    $content .= "            <select style=\"background: rgb(52, 64, 85) none repeat scroll 0%; width: 230px; font-size: 11px; color: white; border-color: #D4D4D4;\" onChange=\"return change(this.options[this.selectedIndex].value);\">\r\n";
    for($i=0,$maxi=count($links);$i<$maxi;$i++) {
        $link = explode('|', $links[$i]);
        $title = $link[0];
        $url = $link[1];
        $content .= "                <option value=\"" . $url . "\">" . $title . "</option>\r\n";
    }
    $content .= "            </select>\r\n";
    $content .= "         \r\n";
    $content .= "        </td>\r\n";
