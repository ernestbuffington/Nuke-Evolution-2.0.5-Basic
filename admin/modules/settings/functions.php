...
function settings_header() {
    ...
    echo "<tr>\n";
    echo "<td align='center' colspan='3'><a href=\"$admin_file.php?op=Configure\"><strong>" . _SITECONFIG . "</strong></a></td>\n";
    echo "</tr>\n";
    echo "<tr>\n";
    echo "<td align='center' width='33%'><a href=\"$admin_file.php?op=Configure&sub=1\">" . _GENSITEINFO . "</a></td>\n";
    echo "<td align='center' width='33%'><a href=\"$admin_file.php?op=Configure&sub=4\">" . _FOOTERMSG . "</a></td>\n";
    echo "<td align='center' width='33%'><a href=\"$admin_file.php?op=Configure&sub=7\">" . _COMMENTSOPT . "</a></td>\n";    
    echo "</tr>\n";
    echo "<tr>";
    echo "<td align='center' width='33%'><a href=\"$admin_file.php?op=themes\">" . _THEMES_THEMES . "</a></td>\n";
    echo "<td align='center' width='33%'><a href=\"$admin_file.php?op=Configure&sub=3\">" . _MULTILINGUALOPT . "</a></td>\n";
    echo "<td align='center' width='33%'><a href=\"$admin_file.php?op=Configure&sub=8\">" . _GRAPHICOPT . "</a></td>\n";

    echo "</tr>\n";
    echo "<tr>\n";
    echo "<td align='center' width='33%'><a href=\"$admin_file.php?op=Configure&sub=9\">" . _MISCOPT . "</a></td>\n";
    echo "<td align='center' width='33%'><a href=\"$admin_file.php?op=Configure&sub=11\">" . _META_TAGS . "</a></td>\n";
    echo "<td align='center' width='33%'><a href=\"$admin_file.php?op=Configure&sub=6\">" . _MAIL2ADMIN . "</a></td>\n";
    echo "</tr>\n";
    echo "<tr>\n";
    echo "<td align='center' width='33%'><a href=\"$admin_file.php?op=Configure&sub=10\">" . _GFXOPT . "</a></td>\n";
    echo "<td align='center' width='33%'><a href=\"$admin_file.php?op=Configure&sub=5\">" . _BACKENDCONF . "</a></td>\n";
    echo "<td align='center' width='33%'><a href=\"$admin_file.php?op=Configure&sub=2\">" . _CENSOR . "</a></td>\n";
    echo "</tr>\n";
...
function show_settings($sub) {
    ...
    OpenTable();
    echo "<div align=\"center\">\n<a href=\"$admin_file.php?op=Configure\">" . _SETTINGS_ADMIN_HEADER . "</a></div>\n";
    echo "<br /><br />";
    echo "<div align=\"center\">\n[ <a href=\"$admin_file.php\">" . _SETTINGS_RETURNMAIN . "</a> ]</div>\n";
    CloseTable();
    echo "<br />";
    settings_header();
    if($sub) {
...
function save_settings($sub) {
    ...
    switch($sub) {
        ...
        case 1:
            ...
            Validate($xadminmail, 'email', 'Nuke Settings', 0, 1, 0, 0, '', '<br /><center>'. _GOBACK .'</center>');
            ...
            if($xnukeurl != $server_url && empty($confirm)) {
                include_once(NUKE_BASE_DIR."header.php");
                OpenTable();
                echo "<center>". sprintf(_URL_SERVER_ERROR, $xnukeurl,$server_url) ."<br /><br />"._URL_CONFIRM_ERROR;
        ...
        case 2:
            ...
            $db->sql_query("UPDATE ".$prefix."_evolution SET evo_value='".$xcensor."' WHERE evo_field='censor'");
            $xcensor_words = str_replace("\n"," ", $_POST['xcensor_words']);
            $db->sql_query("UPDATE ".$prefix."_evolution SET evo_value='".$xcensor_words."' WHERE evo_field='censor_words'");
        break;
 ...
 ******************************************************/
            $xinfobar = intval($_POST['xinfobar']);
/*****[END]********************************************
 [ Mod:     Switch InfoBar on Top              v1.0.0 ]
 ******************************************************/
            ...
            $db->sql_query("UPDATE ".$prefix."_evolution SET evo_value='".$xadminssl."' WHERE evo_field='adminssl'");
/*****[BEGIN]******************************************
 [ Mod:     Switch InfoBar on Top              v1.0.0 ]
 ******************************************************/
            $db->sql_query("UPDATE ".$prefix."_evolution SET evo_value='".$xinfobar."' WHERE evo_field='infobar'");
/*****[END]********************************************
 [ Mod:     Switch InfoBar on Top              v1.0.0 ]
 ******************************************************/
            $db->sql_query("UPDATE ".$prefix."_evolution SET evo_value='".$xqueries_count."' WHERE evo_field='queries_count'");
            $db->sql_query("UPDATE ".$prefix."_evolution SET evo_value='".$xuse_colors."' WHERE evo_field='use_colors'");
            $db->sql_query("UPDATE ".$prefix."_evolution SET evo_value='".$xlock_modules."' WHERE evo_field='lock_modules'");
            $db->sql_query("UPDATE ".$prefix."_evolution SET evo_value='".$xtextarea."' WHERE evo_field='textarea'");
            $db->sql_query("UPDATE ".$prefix."_evolution SET evo_value='".$xlazytap."' WHERE evo_field='lazy_tap'");
            $db->sql_query("UPDATE ".$prefix."_evolution SET evo_value='".$ximg_resize."' WHERE evo_field='img_resize'");
            $db->sql_query("UPDATE ".$prefix."_evolution SET evo_value='".$ximg_width."' WHERE evo_field='img_width'");
            $db->sql_query("UPDATE ".$prefix."_evolution SET evo_value='".$ximg_height."' WHERE evo_field='img_height'");
            $db->sql_query("UPDATE ".$prefix."_evolution SET evo_value='".$xcollapse."' WHERE evo_field='collapse'");
            $db->sql_query("UPDATE ".$prefix."_evolution SET evo_value='".$xcollapsetype."' WHERE evo_field='collapsetype'");
        break;

        case 10:
            $xusegfxcheck = intval($_POST['xusegfxcheck']);
            $db->sql_query("UPDATE ".$prefix."_evolution SET evo_value='".$xusegfxcheck."' WHERE evo_field='usegfxcheck'");
            if (!GDSUPPORT) {
                break;
            }
            if(!defined('CAPTCHA')) {
                ...
                $db->sql_query("UPDATE ".$prefix."_evolution SET evo_value='".$xcodesize."' WHERE evo_field='codesize'");
                $db->sql_query("UPDATE ".$prefix."_evolution SET evo_value='".$xcodefont."' WHERE evo_field='codefont'");
                $db->sql_query("UPDATE ".$prefix."_evolution SET evo_value='".$xuseimage."' WHERE evo_field='useimage'");
            } else {
