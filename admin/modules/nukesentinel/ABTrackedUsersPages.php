...
echo "<form method='post' action='".$admin_file.".php?op=ABTrackedUsersPages'>\n";
...
echo "<tr>";
echo "<td bgcolor='$bgcolor2' nowrap width='70%'><strong>"._AB_PAGEVIEWED."</strong></td>";
echo "<td bgcolor='$bgcolor2' nowrap width='20%'><strong>"._AB_HITDATE."</strong></td>";
echo "<td bgcolor='$bgcolor2' nowrap width='10%'><strong>&nbsp;</strong></td></tr>";
$result = $db->sql_query("SELECT `user_id`, `ip_addr`, `page`, `date` FROM `".$prefix."_nsnst_tracked_ips` WHERE `user_id`='$user_id' ORDER BY $column $direction LIMIT $min, $perpage");
while(list($luserid, $lipaddr, $page, $date_time) = $db->sql_fetchrow($result)){
  ...
  echo "<td align='center'><a href='".$admin_file.".php?op=ABTrackedDeleteSave&amp;tid=$ltid&amp;user_id=$user_id&amp;ip_addr=$ip_addr&amp;column=$column&amp;direction=$direction&amp;min=$min'><img src='images/nukesentinel/delete.png' height='16' width='16' border='0' alt='' title='' /></a></td>";
  echo "</tr>\n";
}
$totalselected = $db->sql_numrows($db->sql_query("SELECT * FROM `".$prefix."_nsnst_tracked_ips` WHERE `user_id`='$user_id'"));
  // Page Numbering
  ...
  if($pages != 1 && $pages != 0) {
    ...
    echo "<td width='33%'>";
    echo "<form action='".$admin_file.".php?op=ABTrackedUsersPages' method='post'>\n";
