...
if(!isset($min)) {
  ...
  while(list($xip_addr) = $db->sql_fetchrow($result)) {
    ...
    list($xc2c) = $db->sql_fetchrow($db->sql_query("SELECT `c2c` FROM `".$prefix."_nsnst_ip2country` WHERE `ip_lo`<='$xip_long' AND `ip_hi`>='$xip_long'"));
    if(!$xc2c) { $xc2c = "00"; }
  ...
  title($pagetitle);
  OpenTable();
  echo "<META HTTP-EQUIV=\"refresh\" content=\"$pagedelay;URL=".$admin_file.".php?op=ABIP2CountryUpdateBlocked&min=$max\">";
  ...
  echo "<br />\n";
  OpenTable();
  echo "<center><strong>"._AB_IP2CUPDATEBLOCKED." "._AB_COMPLETED."</strong><br /></center>\n";
  CloseTable();
  include(NUKE_BASE_DIR."footer.php");
}
