...
echo "<form method='post' action='".$admin_file.".php?op=ABTrackedRefersPages'>\n";
...
$totalselected = $db->sql_numrows($db->sql_query("SELECT * FROM `".$prefix."_nsnst_tracked_ips` WHERE `refered_from`='$uname'"));
  ...
  if($pages != 1 && $pages != 0) {
    ...
    echo "<td width='33%'>";
    echo "<form action='".$admin_file.".php?op=ABTrackedRefersPages' method='post'>\n";
